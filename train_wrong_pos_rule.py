...
def main(args):
    ...
    objects_for_rules=[['bolt'], ['bolt'], ['bolt'], ['hexagonal nut'], ['round washer']]
    texts_for_rules=[['two bolts'], ['short bolts'], ['long bolt'], ['hexagonal nut'], ['round washer']]
    ...
    if args.num_rule == 0:
        ...
        rules_for_data = get_rule(data_name = 'blue_splicing', rule_idxs=[i for i in range(args.num_rule)])
    print(rules_for_data)
    ...
    if args.detection != 1:
        ...
        if args.preprocess != 1:
            ...
            if args.ris == -1:
                ...
                train_dataset = torch.load(os.path.join(base_save_path, "aug_train.pt"))
                val_dataset = torch.load(os.path.join(base_save_path, 'aug_val.pt'))
                test_good_dataset = torch.load(os.path.join(base_save_path, 'aug_test_good.pt'))
                test_la_dataset = torch.load(os.path.join(base_save_path, 'aug_test_la.pt'))
                test_sa_dataset = torch.load(os.path.join(base_save_path, 'aug_test_sa.pt'))
            elif args.ris == 0:
                if os.path.exists(os.path.join(base_save_path, "save")):
                    train_dataset = torch.load(os.path.join(base_save_path, "save/train.pt"))
                    val_dataset = torch.load(os.path.join(base_save_path, 'save/val.pt'))
                    test_good_dataset = torch.load(os.path.join(base_save_path, 'save/test_good.pt'))
                    test_la_dataset = torch.load(os.path.join(base_save_path, 'save/test_la.pt'))
                    test_sa_dataset = torch.load(os.path.join(base_save_path, 'save/test_sa.pt'))
                else:
