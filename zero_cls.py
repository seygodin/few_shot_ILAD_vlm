...
def evaluate(args, model, tokenizer, pairs, test_good_dataset, test_sa_dataset, test_la_dataset):
    ...
    for rule_idx, pair in enumerate(pairs):
        ...
        if args.detection != 1:
            ...
            good_meta = test_good_dataset[:len(test_good_dataset)][2]
            la_meta = test_la_dataset[:len(test_la_dataset)][2]


            #print("Evaluation")
            #print(f"Good Sample meta feature: {good_meta.shape}")
            #print(f"LA Sample meta feature: {la_meta.shape}")
            good_meta_feature_for_rule = good_meta[:, rule_idx, :]
            la_meta_feature_for_rule = la_meta[:, rule_idx, :]
            #print(f"good_meta_feature_for_rule: {good_meta_feature_for_rule.shape}")
            #print(f"la_meta_feature_for_rule: {la_meta_feature_for_rule.shape}")
            good_meta_feature_for_rule = torch.cat((good_meta_feature_for_rule, good_img_features), dim=1)
            la_meta_feature_for_rule = torch.cat((la_meta_feature_for_rule, la_img_features), dim=1)

            good_img_features = meta_projecter(good_meta_feature_for_rule)
            la_img_features = meta_projecter(la_meta_feature_for_rule)
...
def main(args):
    ...
    juice_class_names = ["banana_juice", "orange_juice", "cherry_juice"]
    juice_clip_class_names = ["a photo of white banana juice", "a photo of yellow orange juice", "a photo of red cherry juice"]
    
    splicing_class_names = ["blue_splicing", "red_splicing", "yellow_splicing"]
    splicing_clip_class_names = ["a photo of blue cable", "a photo of red cable", "a photo of yellow cable"]

    if args.data_name in splicing_class_names + juice_class_names:
        correct = 0

        if args.data_name in juice_class_names:
            target_class_names = juice_class_names
            target_clip_class_names = juice_clip_class_names
        elif args.data_name in splicing_class_names:
            target_class_names = splicing_class_names
            target_clip_class_names = splicing_clip_class_names

        zero_cls_clip_model, _, _ = open_clip.create_model_and_transforms('ViT-g-14', pretrained='laion2b_s12b_b42k')
        zero_cls_clip_model= zero_cls_clip_model.to(device)
        zero_cls_clip_tokenizer = open_clip.get_tokenizer('ViT-g-14')
        zero_cls_answer_idx = target_class_names.index(args.data_name)

        with torch.no_grad(), torch.cuda.amp.autocast():
            text = zero_cls_clip_tokenizer(target_clip_class_names)
            text = text.to(device)
            text_features = zero_cls_clip_model.encode_text(text)
            text_features /= text_features.norm(dim=-1, keepdim=True)

        for index, img in enumerate(train_dataset):
            raw_image, _ = img
            with torch.no_grad(), torch.cuda.amp.autocast():
                image_features = zero_cls_clip_model.encode_image(raw_image.unsqueeze(0))
                image_features /= image_features.norm(dim=-1, keepdim=True)
                text_probs = (100.0 * image_features @ text_features.T).softmax(dim=-1)
                class_idx = torch.argmax(text_probs).detach().cpu().numpy()

            if zero_cls_answer_idx == class_idx:
                correct += 1
            else:
                print(train_dataset.img_paths[index], " -> ", target_class_names[class_idx])

        print(f"Zero-Shot Classification Accuracy: {correct}/{len(train_dataset)} | {correct/len(train_dataset)*100}%")
        sys.exit()
