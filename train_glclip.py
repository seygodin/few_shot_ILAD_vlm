...
def main(args):
    ...
    if args.detection != 1:
        ...
        if args.preprocess != 1:
            ...
            base_save_path = os.path.join("clip_detection", args.data_name)
            train_dataset = torch.load(os.path.join(base_save_path, "clip_train.pt"))
            val_dataset = torch.load(os.path.join(base_save_path, 'clip_val.pt'))
            test_good_dataset = torch.load(os.path.join(base_save_path, 'clip_test_good.pt'))
            test_la_dataset = torch.load(os.path.join(base_save_path, 'clip_test_la.pt'))
