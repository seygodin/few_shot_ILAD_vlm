...
def evaluate(args, model, tokenizer, pairs, test_good_dataset, test_sa_dataset, test_la_dataset):
    ...
    good_img_paths = test_good_dataset.img_paths
    la_img_paths = test_la_dataset.img_paths
    ...
    for i in range(len(good_probs)):
        print(good_img_paths[i], good_probs[i])
    for i in range(len(la_probs)):
        print(la_img_paths[i], la_probs[i])
    """
    ...
    auprc, optimal_f1, optimal_threshold = calculate_auprc_and_optimal_f1(true_label=la_labels, pred_score=la_img_pred)
    #print(f"Pred: {sa_img_pred}")
    #print(f"Label: {sa_labels}")
    #set_trace()
    sa_auc = 0

    la_img_pred[la_img_pred >= optimal_threshold] = 1
    la_img_pred[la_img_pred < optimal_threshold] = 0
    pred_accuracy = accuracy_score(la_img_pred, la_labels) 
    return sa_auc, la_auc, auprc, optimal_f1, pred_accuracy
...
def train(args, model, tokenizer, pairs, train_dataloader, valid_dataloader, test_good_dataset, test_sa_dataset, test_la_dataset, optimizer, epoch=1, tag="default"):
    ...
    for i in range(epoch):
        ...
        sa_auc, la_auc, la_auprc, la_f1_max, la_acc = evaluate(args, model, tokenizer, pairs, test_good_dataset, test_sa_dataset, test_la_dataset)
        if args.log:
            print(f"[Test ROC-AUC]  -  LA: {la_auc:.5f}")
            print(f"[Test AUPRC]  -  LA: {la_auprc:.5f}")
            print(f"[Test F1-max]  -  LA: {la_f1_max:.5f}")
            print(f"[Test Accuracy]  -  LA: {la_acc:.5f}")
    ...
    print(f"Best LA Accuracy: {best_la_acc}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--kernel_size', default=250, required=False, type=int)
    parser.add_argument('--overlap_count', default=8, required=False, type=int)
